...
export class RdfController {
  ...
  async createEntity(@Body() createEntityDto: Partial<IFCEntity>): Promise<IFCEntityResult> {
    try {
  ...
  @ApiQuery({ name: 'name_filter', required: false, description: 'Filter by name (partial match)' })
  @ApiQuery({ name: 'limit', required: false, description: 'Limit results', type: Number })
  @ApiQuery({ name: 'skip', required: false, description: 'Skip results for pagination', type: Number })
  async getEntities(@Query() query: IFCEntityListRequest): Promise<IFCEntityResponse> {
    try {
  ...
  @ApiParam({ name: 'entityType', description: 'IFC entity type (e.g., IFCSpace, IFCWall)' })
  @ApiResponse({ 
    status: 200, 
    description: 'Entities of specified type retrieved successfully',
    type: [IFCEntityResult]
  })
  async getEntitiesByType(@Param('entityType') entityType: string): Promise<IFCEntityResult[]> {
    try {
  ...
  async updateEntity(
    ...
    try {
      const updatedEntity = await this.rdfService.updateEntity(id, updateEntityDto);
      if (!updatedEntity) {
        throw new HttpException('Entity not found', HttpStatus.NOT_FOUND);
      }
  ...
  @ApiResponse({ status: 400, description: 'Bad request' })
  async createBulkEntities(@Body() entities: Partial<IFCEntity>[]): Promise<IFCEntityResult[]> {
    try {
  ...
  async searchEntities(@Param('searchTerm') searchTerm: string): Promise<IFCEntityResult[]> {
    try {
