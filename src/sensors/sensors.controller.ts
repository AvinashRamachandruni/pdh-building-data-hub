...
import { ApiBearerAuth, ApiDefaultResponse, ApiOperation, ApiParam, ApiQuery, ApiTags } from '@nestjs/swagger'
...
export class SensorsController {
  ...
  @ApiQuery({
    ...
    example: 100,
    type: Number,
    description: 'if empty, returns all the data, otherwise returns a random sample of 100 points between the specified time period.'
  })
  async getRecords(
    ...
    return {
      ...
      result: await this.sensorsService
        .getRecords(sensor_id, start, end, Number(sample))
    }
